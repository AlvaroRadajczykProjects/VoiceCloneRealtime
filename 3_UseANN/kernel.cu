...
static int patestCallback(
	...
	float* vi = (float*)inputBuffer;
	float* vo = (float*)outputBuffer;

	//for (int i = 0; i < FRAMES_PER_BUFFER; i++) { vs[i] = (i + 1) / (float)FRAMES_PER_BUFFER; }

	n->forward(1, vi, vo);

	//memcpy(vi, vo, FRAMES_PER_BUFFER * sizeof(float));
...
int main() {
	...
	int input_device = 36; //linea 1 de entrada (name: Line 1 (Virtual Cable 1) maxInputChannels: 2 maxOutputChannels: 0), debe de iniciarse el audio repeater con entrada micrófono y salida la linea 1
	int output_device = 42; //linea 2 de salida (name: Line Out (Virtual Cable 2) maxInputChannels: 0 maxOutputChannels: 2)
	...
	*/
	...
	n = new Network(FRAMES_PER_BUFFER, 1, 3, new Layer * [3] {
